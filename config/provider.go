...
func GetProvider() *config.Provider {
	pc := config.NewProvider(
		...
		config.WithTerraformPluginSDKIncludeList([]string{
			// This path corresponds to the internal provider package structure
			// of the garage-terraform-provider. If the upstream provider structure
			// changes, this path may need to be updated.
